# Second pass over AuditLog.EventType: 0003 caught the events the app was emitting
# at the time, but six were still undeclared — the five AI failure/analysis types in
# apps/ai/views.py (including _upstream_failure's AI_REQUEST_FAILED default) and
# OFFLINE_SYNC_REJECTED in apps/core/views.py. Same shape as 0003: choices is not a
# database constraint, so this is a state-only AlterField with no data migration and
# the column keeps max_length=40. Without it these rows stayed invisible to the admin
# filters and to get_event_type_display().
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('audit', '0003_auditlog_event_type_choices'),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.CharField(choices=[
                ...
                ('AI_IMAGE_ANALYSIS', 'تحليل صورة طبية'),
                ('AI_IMAGE_ANALYSIS_FAILED', 'فشل تحليل صورة طبية'),
                ('AI_STRUCTURE_NOTE_FAILED', 'فشل هيكلة ملاحظة سريرية'),
                ('AI_TRIAGE_FAILED', 'فشل التقييم الآلي للخطورة'),
                ('AI_REQUEST_FAILED', 'فشل طلب ذكاء اصطناعي'),
                ('OFFLINE_SYNC_REJECTED', 'رفض مزامنة دون اتصال'),
